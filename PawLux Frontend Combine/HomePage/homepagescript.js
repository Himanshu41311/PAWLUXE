document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!isLoggedIn || !currentUser) {
    ...
    welcomeText.textContent = `Welcome, ${currentUser.user.fullName || "User"}!`;
    welcomeText.style.cssText = "margin-right: 10px; font-weight: bold; color: white;";
    ...
    animals.forEach((animal, index) => {
      ...
      card.innerHTML = `
        ...
        <div class="dog-description">
          ...
          <strong>Description:</strong> ${sanitize(animal.description)}<br>
          <strong>Price:</strong> ${sanitize(animal.price)} INR<br>
        </div>

        <div class="dog-footer">
          <div class="owner-detail"><strong>Owner:</strong> ${sanitize(animal.ownerName)}, ${sanitize(animal.ownerState)}</div>
  ...
  async function onSearchClick() {
    ...
    try {
      const response = await fetch("http://localhost:8081/view/data/conditions", {
        method: "POST",
      ...
      console.error(err);
      content.innerHTML = `<p style="color:red;">Something went wrong. Please try again later.</p>`;
    }
  }
});
